...
begin
  ...
  if (l = 0) then
    checkWeg := true // l=0 ist True
  else if (l = n) then
    checkWeg := (s[1] = 'a') and (s[l] = 'a') and (a = 2) and (b = 1) and
      (c = 1) and (d = 1) and (e = 1) // l=n ist 2x a und 1x Rest
  else
    checkWeg := (s[1] = 'a') and (a <= 1) and (b <= 1) and (c <= 1) and
      (d <= 1) and (e <= 1); // l<>0 und l<>n jeder Buchstabe maximal einmal, Rekursion weiterf�hren
end;
